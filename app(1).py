...
def _wrapper_agregar_y_entrenar(archivos, nombre, progress=gr.Progress()):
    """Todo automático: subís, se organiza (con recorte de grilla automático
    y control de calidad) y, si ya hay suficientes imágenes, se reentrena el
    LoRA — sin botones intermedios. OJO: esto significa que cada vez que
    subís algo se relanza un entrenamiento completo (15-40+ min), no solo
    cuando vos decidís que ya juntaste suficiente material."""
    try:
        if not archivos:
        ...
        # es_grilla=True siempre: detecta paneles automáticamente si la imagen
        # es una hoja con varias poses, y si es una sola foto normal, la
        # función ya devuelve la imagen completa como un único panel — no
        # hace falta que el usuario aclare cuál es cuál.
        ...
        if n_imagenes < 3:
            return (f"{resumen_dataset}\n\n"
                    f"⏳ Todavía no se entrena: hacen falta al menos 3 imágenes en total "
                    f"(hay {n_imagenes}). Seguí subiendo.")

        token = f"{nombre_norm}_chr"
        entrenar_lora_personaje(nombre_norm, carpeta, token, pasos=1200, rank=32, progress=progress)
        return f"{resumen_dataset}\n\n✅ LoRA de '{nombre_norm}' reentrenado automáticamente con {n_imagenes} imágenes."
    except gr.Error:
        raise
    except Exception:
    ...
    with gr.Tab("2️⃣ Generar imágenes"):
        ...
        with gr.Accordion("🛠️ Corregir y volver a generar", open=True):
            ...
            with gr.Row():
                ...
                boton_correcta_g = gr.Button("✅ Está correcta")
        ...
        boton_correcta_g.click(
            marcar_ultima_correcta, [estado_ultima_generacion], [log_correccion_g]
        )
