...
# ──────────────────────────────────────────────────────────────────────────
# Paso 6: escalado final a 4K real (misma función PyTorch/GPU del paso 1)
# ──────────────────────────────────────────────────────────────────────────
def escalar_video_a_4k(ruta_video_entrada, ruta_video_salida):
    ...
    for carpeta in (carpeta_in, carpeta_out):
    ...
    nombres = sorted(os.listdir(carpeta_in))
    for i, nombre in enumerate(nombres):
        img = Image.open(os.path.join(carpeta_in, nombre)).convert("RGB")
        img = escalar_con_esrgan(img)
        img = img.resize((ANCHO_4K, ALTO_4K), Image.LANCZOS)
        img.save(os.path.join(carpeta_out, nombre))
        if i % 10 == 0:
            print(f"Escalados {i}/{len(nombres)} frames a 4K...")
