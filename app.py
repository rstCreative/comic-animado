...
# ──────────────────────────────────────────────────────────────────────────
# Persistencia en Google Drive (si está montado): sin esto, TODO lo que
# arma esta app en /content se pierde al reiniciar la sesión de Colab —
# no solo el modelo base (~40GB), también el dataset que vas subiendo de a
# poco y los LoRAs ya entrenados. Con Drive montado en /content/drive, todo
# eso vive ahí y sobrevive entre sesiones.
...
if _DRIVE_MONTADO:
    ...
    os.environ.setdefault("HF_HOME", os.path.join(_BASE_DRIVE, "qwen_image_cache"))
    print(f"[Cache] Google Drive detectado — modelo, dataset y LoRAs se guardan en {_BASE_DRIVE} "
          f"(sobreviven al reiniciar la sesión).", flush=True)
else:
    print("[Cache] Google Drive NO está montado — todo (modelo de ~40GB, dataset, LoRAs entrenados) "
          "se guarda en el disco temporal de Colab y SE PIERDE al reiniciar la sesión. Montá Drive "
          "antes de correr esto si querés que persista (ver README).", flush=True)
