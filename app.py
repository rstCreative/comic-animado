...
def animar_rostro_con_audio(ruta_imagen, ruta_audio, nombre_salida):
    ...
    try:
        print(f"  → Preparando SadTalker para {nombre_salida} (puede tardar la primera vez)...", flush=True)
        _preparar_sadtalker()
        print(f"  → Generando rostro animado para {nombre_salida}...", flush=True)
...
def animar_escena(ruta_imagen, nombre_salida, duracion=3.0, fps=24, amplitud_px=22, lado_max_trabajo=1280):
    ...
    o None si falla."""
    try:
        print(f"  → Cargando imagen para paralaje ({nombre_salida})...", flush=True)
        img_pil = Image.open(ruta_imagen).convert("RGB")

        # El panel ya viene agrandado x4 del paso de preprocesamiento — procesar
        # frame a frame a ese tamaño es muy lento y no aporta nada (el resultado
        # se reescala al lienzo del video de todas formas). Se trabaja a un
        # tamaño razonable y se reescala al final.
        if max(img_pil.size) > lado_max_trabajo:
            escala = lado_max_trabajo / max(img_pil.size)
            nuevo_tam = (round(img_pil.width * escala), round(img_pil.height * escala))
            img_pil = img_pil.resize(nuevo_tam, Image.LANCZOS)

        print(f"  → Estimando profundidad ({nombre_salida})...", flush=True)
        ...
        print(f"  → Generando {n_frames} frames de paralaje ({nombre_salida})...", flush=True)
        frames = []
        for i in range(n_frames):
            ...
            frames.append(frame)
            if (i + 1) % 20 == 0:
                print(f"    ...{i+1}/{n_frames} frames listos", flush=True)

        print(f"  → Codificando video de escena ({nombre_salida})...", flush=True)
        ...
        print(f"  → Escena lista ({nombre_salida}).", flush=True)
        return destino
    except Exception as e:
