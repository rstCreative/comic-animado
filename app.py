...
# ──────────────────────────────────────────────────────────────────────────
def _asegurar_vulkan_ve_la_gpu():
    """Sin esto, Colab suele quedarse con un renderizador por software (llvmpipe),
    increíblemente lento. Esto le da a Vulkan el archivo que le falta para usar
    la GPU NVIDIA real (workaround conocido y documentado para Colab)."""
    icd_path = "/usr/share/vulkan/icd.d/nvidia_icd.json"
    if os.path.exists(icd_path) or not torch.cuda.is_available():
        return
    ...
    try:
        with open(icd_path, "w") as f:
            f.write(contenido)
        print("Vulkan configurado para usar la GPU NVIDIA (no CPU).")
    except PermissionError:
        subprocess.run(["sudo", "tee", icd_path], input=contenido.encode(), check=True)


def preparar_realesrgan():
    global _binario_esrgan
    _asegurar_vulkan_ve_la_gpu()
