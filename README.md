...
## Requisitos
- Python 3.10+ (Linux recomendado; el escalado 4K usa un ejecutable solo para Linux).
- GPU recomendada (no obligatoria, pero mucho más lento en CPU).
- `ffmpeg` instalado en el sistema (`apt-get install ffmpeg` en Linux).

## Uso en Google Colab
```python
!pip install -q -r requirements.txt
!apt-get install -y -qq ffmpeg libvulkan1 mesa-vulkan-drivers
!python app.py
```
...
```bash
sudo apt-get install -y ffmpeg libvulkan1 mesa-vulkan-drivers   # una sola vez
