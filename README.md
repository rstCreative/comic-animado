...
**Paso 1 — montá Google Drive** (para no perder el modelo de ~40GB, el dataset
ni los LoRAs entrenados cada vez que se reinicia la sesión):
...
**Paso 2 — cloná y corré** (el `%cd /content` de arranque evita el bug de
clonar el repo anidado adentro de una carpeta `comic-animado` que haya
quedado de una sesión anterior):

```python
%cd /content
!rm -rf comic-animado
!git clone https://github.com/rstCreative/comic-animado.git
%cd /content/comic-animado
...
Si montaste Drive, vas a ver en el log: `[Cache] Google Drive detectado...`
— eso confirma que el modelo, el dataset y los LoRAs van a persistir entre
sesiones (quedan guardados en `Mi unidad/comic_animado_data/` dentro de tu
Drive). Si no lo montaste, la app funciona igual, pero se pierde todo al
reiniciar la sesión.
