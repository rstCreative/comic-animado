...
# ──────────────────────────────────────────────────────────────────────────
# Persistencia en Google Drive (si está montado).
#
# IMPORTANTE — lección aprendida probando esto: escribir/leer el modelo
# (~40GB, un solo archivo enorme reconstruido de a pedazos) DIRECTO en un
# Drive montado es muchísimo más lento e inestable que en el disco local de
# Colab — el sistema de archivos de Drive no está pensado para ese patrón de
# escritura. Por eso el modelo SIEMPRE se descarga/usa en disco local
# (rápido), y recién DESPUÉS de cargar bien se copia a Drive en un proceso
# de fondo que no bloquea nada — si se corta la sesión a mitad de esa copia,
# no pasa nada grave, la próxima vez arranca de nuevo desde donde haya
# quedado (rsync no vuelve a copiar lo que ya está).
# El dataset y los LoRAs entrenados sí son chicos (imágenes + pesos LoRA,
# no el modelo base) y van directo a Drive sin este problema.
# IMPORTANTE: HF_HOME tiene que fijarse ANTES de importar huggingface_hub/
# diffusers/transformers (son imports diferidos dentro de las funciones de
# carga de modelo, así que llega a tiempo estando acá arriba de todo).
# ──────────────────────────────────────────────────────────────────────────
...
os.environ.setdefault("HF_HOME", _CACHE_LOCAL_MODELO)  # el modelo SIEMPRE local, nunca directo a Drive

if _DRIVE_MONTADO:
    os.makedirs(_BASE_DRIVE, exist_ok=True)
    if os.path.isdir(_CACHE_DRIVE_MODELO) and os.listdir(_CACHE_DRIVE_MODELO):
        print("[Cache] Ya hay un modelo guardado en Drive de una sesión anterior — copiándolo al "
              "disco local antes de arrancar (más rápido que leerlo de Drive en vivo)...", flush=True)
        shutil.copytree(_CACHE_DRIVE_MODELO, _CACHE_LOCAL_MODELO, dirs_exist_ok=True)
        print("[Cache] Copia local lista.", flush=True)
    print(f"[Cache] Google Drive detectado — dataset y LoRAs se guardan ahí (chicos, sin problema). "
          f"El modelo base se maneja en disco local y se sincroniza a Drive en segundo plano después "
          f"de la primera carga exitosa, sin bloquear la generación.", flush=True)
else:
    print("[Cache] Google Drive NO está montado — todo se guarda en el disco temporal de Colab y se "
          "pierde al reiniciar la sesión. Montá Drive antes de correr esto si querés que persista.",
          flush=True)
...
def _sincronizar_modelo_a_drive_en_fondo():
    """Copia el cache local del modelo a Drive en un hilo aparte, sin
    bloquear la generación — así la próxima sesión no tiene que descargar
    todo de nuevo, pero ESTA sesión no se queda esperando a que termine.
    Usa shutil (Python puro) en vez de rsync para no depender de que el
    ...
    def _copiar():
        try:
            print("[Cache] Sincronizando el modelo a Drive en segundo plano (no bloquea nada, "
                  "podés seguir generando mientras tanto)...", flush=True)
            shutil.copytree(_CACHE_LOCAL_MODELO, _CACHE_DRIVE_MODELO, dirs_exist_ok=True)
            print("[Cache] Sincronización a Drive completa.", flush=True)
        except Exception as e:
            print(f"⚠️ Falló la sincronización a Drive en segundo plano (no afecta la generación "
                  f"en esta sesión, solo significa que la próxima sesión puede tener que descargar "
                  f"de nuevo): {e}", flush=True)
...
CARPETA_SALIDAS = "salidas_generador"  # solo resultados generados, no hace falta persistirlos
...
# ──────────────────────────────────────────────────────────────────────────
def _imagen_cv2_desde_archivo(ruta_imagen):
    """Carga imágenes incluso cuando OpenCV no reconoce directamente el formato
    (por ejemplo algunos WEBP/AVIF/GIF), normalizándolas desde PIL."""
    try:
        ...
        pass
    try:
        pil = Image.open(ruta_imagen).convert("RGB")
        return cv2.cvtColor(np.array(pil), cv2.COLOR_RGB2BGR)
    except Exception as e:
        raise gr.Error(f"No se pudo leer la imagen '{os.path.basename(ruta_imagen)}': {e}")
...
def _detectar_paneles_simple(ruta_imagen, area_min_frac=0.01):
    ...
    img = _imagen_cv2_desde_archivo(ruta_imagen)
    h, w = img.shape[:2]
...
def agregar_al_dataset(nombre_personaje, archivos_subidos, descripcion_base, es_grilla, progress=gr.Progress()):
    ...
    Cada panel pasa por el control de calidad como ADVERTENCIA, pero no se
    descarta ni se mueve fuera del dataset principal. Esto permite subir
    material variado y conservarlo para revisión manual. Los motivos de QC
    se registran en el resumen y junto al caption cuando corresponde."""
    ...
    # Se conserva por compatibilidad, pero el flujo normal ya no mueve
    # automáticamente imágenes sospechosas a esta carpeta.
    ...
    for idx_archivo, ruta_subida in enumerate(archivos_subidos):
        ...
        img_original = _imagen_cv2_desde_archivo(ruta_subida)
        ...
        for img in recortes:
            ...
            # QC abierto: TODO material subido se conserva en el dataset.
            destino_carpeta = carpeta_personaje
            ...
            with open(os.path.join(destino_carpeta, nombre_archivo.replace(".png", ".txt")), "w") as f:
                caption = f"{token}, {descripcion_base}".strip(" ,")
                if motivos:
                    caption += f" | QC_ADVERTENCIA: {'; '.join(motivos)}"
                f.write(caption)
            agregadas += 1
            if motivos:
                a_revisar += 1
                print(f"⚠️ {nombre_archivo}: {', '.join(motivos)} → conservada en dataset (QC abierto)", flush=True)
    ...
    if a_revisar:
        resumen += f"⚠️ {a_revisar} tienen advertencias del QC, pero fueron conservadas en el dataset principal. Revisalas manualmente antes de entrenar.\n"
...
def _cargar_pipeline(nombre_personaje):
    ...
    if _pipe_actual is None:
        ...
        _sincronizar_modelo_a_drive_en_fondo()
    else:
...
_TERMINOS_REVELADORES = {
    "bikini", "swimsuit", "swimwear", "underwear", "panties", "bra", "thong",
    "topless", "lingerie", "traje de baño", "ropa interior", "tanga", "bragas",
}
...
def _prompt_libre_es_seguro(texto):
    ...
    tiene_revelador = any(term in t for term in _TERMINOS_REVELADORES)
    return not (tiene_menor and (tiene_sexual or tiene_revelador))
...
def construir_prompt(token_identificador, descripcion_base, genero, edad,
    ...
    # Modo prompt libre: si hay texto acá, se ignoran TODOS los dropdowns de
    # abajo (ropa, pose, mascota, etc.) — se usa esto tal cual, con el
    # personaje ya puesto (token + descripción base) y el estilo/fondo según
    # corresponda. El freno de edad+ropa reveladora de ROPA_SOLO_ADULTO no
    # aplica acá porque no hay dropdown de ropa involucrado, pero el
    # guardrail de palabras clave sí — cubre también ropa reveladora, no
    # solo términos explícitamente sexuales.
    if prompt_libre_personaje and prompt_libre_personaje.strip():
        if not _prompt_libre_es_seguro(prompt_libre_personaje) or not _prompt_libre_es_seguro(descripcion_base):
            raise gr.Error("Ese pedido no se puede generar.")
        partes = [token_identificador, descripcion_base, prompt_libre_personaje.strip()]
        texto_combinado = f"{descripcion_base} {prompt_libre_personaje}".lower()
        if any(term in texto_combinado for term in _TERMINOS_MENOR):
            # El dropdown de Edad no interviene en este modo, así que esta
            # reafirmación se agrega siempre que el texto libre mencione un
            # menor — no es opcional ni se puede desactivar desde la UI.
            partes.append("non-sexualized, fully clothed, innocent, wholesome family-friendly style")
...
def generar_personaje(nombre_personaje, token_identificador, descripcion_base,
                       ...
                       activar_control_calidad, prompt_libre_personaje, semilla, num_imagenes, progress=gr.Progress()):
...
def generar_libre(prompt, prompt_negativo_extra, formato, quitar_fondo_bool,
    ...
    try:
        ...
        return rutas, semilla_usada, "✅ Listo."
    except gr.Error:
        raise
    except Exception:
        return [], None, _resumen_error()
...
# ──────────────────────────────────────────────────────────────────────────
def _resumen_error():
    """Traceback completo para mostrar DENTRO de la interfaz — así se puede
    diagnosticar un error sin tener que ir a buscar los logs de Colab
    (que en el celular son un lío de scrollear)."""
    return "❌ Error inesperado — pasale este texto completo a Claude:\n\n" + traceback.format_exc()



# ──────────────────────────────────────────────────────────────────────────
# PASO 5B — Bucle de corrección interactiva de generaciones
# ──────────────────────────────────────────────────────────────────────────
CARPETA_FEEDBACK = os.path.join(_BASE_DRIVE, "feedback_generaciones") if _DRIVE_MONTADO else "feedback_generaciones"
os.makedirs(CARPETA_FEEDBACK, exist_ok=True)


def _construir_prompt_correccion(prompt_original, anomalia):
    """Añade una corrección concreta sin reescribir innecesariamente el prompt."""
    texto = (anomalia or "").strip()
    if not texto:
        raise gr.Error("Escribí qué salió mal para poder regenerar la imagen.")
    return (
        f"{prompt_original.strip()}\n\n"
        "CORRECCIÓN DE CALIDAD OBLIGATORIA:\n"
        f"{texto}\n"
        "Mantener el personaje, identidad visual, ropa, escena, composición y estilo "
        "del pedido original siempre que no entren en conflicto con la corrección. "
        "La anatomía debe ser coherente: una cabeza, dos brazos, dos manos, dos piernas "
        "y cinco dedos por mano cuando las manos sean visibles. No introducir cambios "
        "innecesarios en otros elementos."
    )


def _guardar_feedback(nombre, prompt_original, prompt_corregido, semilla_original,
                       semilla_nueva, anomalia, ruta_original, ruta_corregida):
    carpeta = os.path.join(CARPETA_FEEDBACK, nombre)
    os.makedirs(carpeta, exist_ok=True)
    registro = os.path.join(carpeta, "errores.jsonl")
    import json
    datos = {
        "personaje": nombre,
        ...
        "semilla_nueva": semilla_nueva,
        "anomalia_reportada": anomalia,
...
def corregir_ultima_generacion(estado, anomalia, tipo_salida, agregar_borde,
                               activar_control_calidad, progress=gr.Progress()):
    """Regenera la última imagen usando el mismo contexto y una corrección explícita.
    La corrección se guarda como memoria; no entrena el LoRA automáticamente."""
    try:
        if not estado or not isinstance(estado, dict) or not estado.get("ruta"):
            raise gr.Error("Primero generá una imagen en la pestaña de personajes.")
        nombre = estado["nombre"]
        prompt_original = estado["prompt"]
        semilla_original = int(estado["semilla"])
        prompt_corregido = _construir_prompt_correccion(prompt_original, anomalia)
        semilla_nueva = semilla_original + 900001 + random.randint(0, 9999)

        progress(0.1, desc="Cargando personaje para corregir...")
        pipe = _cargar_pipeline(nombre)
        es_sticker = TIPO_SALIDA[tipo_salida] == "sticker"
        negativo = NEGATIVO_UNIVERSAL + (f", {NEGATIVO_EXTRA_SIN_FONDO}" if es_sticker else "")
        ...
        for intento in range(max_intentos):
            seed = semilla_nueva + intento * 1001
            progress(0.2 + 0.2 * intento, desc=f"Regenerando corrección (intento {intento+1}/{max_intentos})...")
            generador = torch.Generator(device="cpu").manual_seed(seed)
            resultado = pipe(
                prompt=prompt_corregido,
            ...
            candidato = resultado.images[0]
            rgba = quitar_fondo(candidato)
            if activar_control_calidad:
                ok, motivos, detector_faltante = revisar_calidad_imagen(candidato, rgba)
                ultimo_log = motivos
                if ok:
                    mejor = (candidato, rgba, seed)
                    break
            else:
                mejor = (candidato, rgba, seed)
                break
        ...
        candidato, rgba, seed_final = mejor
        imagen_final = rgba if es_sticker else candidato
        if es_sticker and agregar_borde:
            imagen_final = agregar_borde_sticker(imagen_final)
        ruta_nueva = os.path.join(CARPETA_SALIDAS, f"{nombre}_corregida_{seed_final}.png")
        imagen_final.save(ruta_nueva)

        _guardar_feedback(
            nombre, prompt_original, prompt_corregido, semilla_original,
            seed_final, (anomalia or "").strip(), estado["ruta"], ruta_nueva
        )

        log = "✅ Corrección generada y guardada. La experiencia quedó registrada para este personaje."
        if ultimo_log:
            log += "\n⚠️ El QC todavía detectó: " + ", ".join(ultimo_log)
        log += "\nℹ️ La memoria se guarda; el LoRA no se modifica automáticamente con un solo error."
        progress(1.0, desc="Corrección lista.")
        nuevo_estado = dict(estado)
        nuevo_estado.update({"ruta": ruta_nueva, "semilla": seed_final, "prompt": prompt_corregido})
        return [ruta_nueva], seed_final, log, nuevo_estado
    except gr.Error:
        raise
    except Exception:
        return [], None, _resumen_error(), estado


def marcar_ultima_correcta(estado):
    if not estado:
        return "ℹ️ No hay una generación activa para marcar."
    return "✅ Marcada como correcta. Podés usarla como referencia; no se modifica el LoRA automáticamente."

def aceptar_ultima_generacion_como_ejemplo(estado):
    """Guarda la última generación aceptada como ejemplo para futuros reentrenamientos."""
    try:
        if not estado or not estado.get("ruta") or not os.path.isfile(estado["ruta"]):
            raise gr.Error("Primero generá una imagen.")
        nombre = estado["nombre"]
        carpeta = os.path.join(CARPETA_DATASETS, nombre, "ejemplos_aceptados")
        os.makedirs(carpeta, exist_ok=True)
        seed = estado.get("semilla", "sin_seed")
        destino = os.path.join(carpeta, f"aceptada_{seed}.png")
        from shutil import copy2
        copy2(estado["ruta"], destino)
        with open(os.path.splitext(destino)[0] + ".txt", "w", encoding="utf-8") as f:
            f.write(estado.get("prompt", "").strip() + "\n")
        return f"✅ Ejemplo aceptado guardado para '{nombre}'. Se incorporará en el próximo entrenamiento.", estado
    except gr.Error:
        raise
    except Exception:
        return _resumen_error(), estado


def entrenar_actualizar_desde_dataset(nombre, pasos, rank, progress=gr.Progress()):
    try:
        nombre = (nombre or "").strip().lower().replace(" ", "_")
        if not nombre:
            raise gr.Error("Escribí el nombre del personaje.")
        carpeta = os.path.join(CARPETA_DATASETS, nombre)
        if not os.path.isdir(carpeta):
            raise gr.Error(f"No existe todavía el dataset de '{nombre}'.")
        n_imagenes = sum(1 for raiz, _, archivos in os.walk(carpeta) for f in archivos if f.lower().endswith(".png"))
        if n_imagenes < 3:
            raise gr.Error(f"Solo hay {n_imagenes} imágenes. Agregá al menos 3 antes de entrenar.")
        token = f"{nombre}_chr"
        entrenar_lora_personaje(nombre, carpeta, token, pasos=int(pasos), rank=int(rank), progress=progress)
        return f"🧠 LoRA de '{nombre}' actualizado con {n_imagenes} imágenes, incluyendo ejemplos aceptados."
    except gr.Error:
        raise
    except Exception:
...
def _wrapper_agregar_y_entrenar(archivos, nombre, progress=gr.Progress()):
    """Sube y organiza el dataset sin disparar entrenamiento automáticamente."""
    try:
        if not archivos:
            raise gr.Error("Subí al menos una imagen con el botón +.")
        if not nombre or not nombre.strip():
            raise gr.Error("Escribí el nombre del personaje antes de subir imágenes.")
        resumen_dataset = agregar_al_dataset(nombre, archivos, "", True, progress=progress)
        nombre_norm = nombre.strip().lower().replace(" ", "_")
        carpeta = os.path.join(CARPETA_DATASETS, nombre_norm)
        n_imagenes = len([f for f in os.listdir(carpeta) if f.lower().endswith(".png")]) if os.path.isdir(carpeta) else 0
        return f"{resumen_dataset}\n\n📦 Dataset actualizado: {n_imagenes} imágenes. El LoRA NO se entrena automáticamente. Usá '🧠 Entrenar / actualizar LoRA' cuando quieras."
    except gr.Error:
        raise
    except Exception:
        return _resumen_error()
...
def _wrapper_agregar_dataset(nombre, archivos, descripcion_base, es_grilla, progress=gr.Progress()):
    try:
        ...
        raise  # los gr.Error ya son mensajes pensados para el usuario, se muestran solos
    except Exception:
        return _resumen_error()
...
def _parsear_entrada_personaje(texto):
    """'rstchica: sentada en la playa con un perro' -> ('rstchica', 'sentada en la playa con un perro').
    Separa por los PRIMEROS dos puntos — todo lo de antes es el nombre del
    personaje (tiene que ser el mismo que usaste al entrenar), todo lo de
    después es la descripción libre de lo que querés generar."""
    if not texto or ":" not in texto:
        raise gr.Error("Escribí en el formato 'nombre_personaje: qué querés generar' (con los dos puntos). "
                        "Ej: 'rstchica: sentada en la playa con un perro de collar negro'")
    nombre_raw, resto = texto.split(":", 1)
    nombre = nombre_raw.strip().lower().replace(" ", "_")
    prompt = resto.strip()
    if not nombre:
        raise gr.Error("Falta el nombre del personaje antes de los ':'.")
    if not prompt:
        raise gr.Error("Escribí qué querés generar después de los ':'.")
    return nombre, prompt


def _wrapper_generar_simple(entrada, tipo_salida, escenario_preset, fondo_descripcion,
                             agregar_borde, activar_control_calidad, semilla, num_imagenes,
                             progress=gr.Progress()):
    try:
        nombre, prompt = _parsear_entrada_personaje(entrada)
        token = f"{nombre}_chr"
        rutas, semilla_usada, log = generar_personaje(
            nombre, token, "", "Mujer", "Adulto", "Marrón", "Grandes clásicos",
            "Negro", "Mediano", "Liso", ["Ninguno"], "Casual", "Zapatillas/tenis", "", "Ninguna",
            "Ninguna", "Ninguno", "", "Sin especificar",
            "Ninguna (uso el texto de pose libre)", "Ninguno", "Sin especificar", "Sin especificar",
            "Sin especificar", "Sin especificar", "",
            tipo_salida, escenario_preset, fondo_descripcion, agregar_borde, activar_control_calidad,
            prompt, semilla, num_imagenes, progress=progress,
        )
        estado = {
            "nombre": nombre,
            "prompt": prompt,
        ...
        return rutas, semilla_usada, log, estado
    except gr.Error:
        raise
    except Exception:
        return [], None, _resumen_error(), {}
...
def _wrapper_generar(nombre, token, descripcion_base, genero, edad, color_ojos, tipo_ojos, color_pelo,
                      ...
                      expresion, gesto, vista, pose_cuerpo, pose_dinamica, actividad_vehiculo, pose_texto,
                      tipo_salida, escenario_preset, fondo_descripcion, agregar_borde, activar_control_calidad,
                      prompt_libre_personaje, semilla, num_imagenes, progress=gr.Progress()):
    try:
        nombre = nombre.strip().lower().replace(" ", "_")
        if not prompt_libre_personaje or not prompt_libre_personaje.strip():
            raise gr.Error("Escribí qué querés generar en el cuadro de texto.")
        rutas, semilla_usada, log = generar_personaje(
            ...
            expresion, gesto, vista, pose_cuerpo, pose_dinamica, actividad_vehiculo, pose_texto,
            tipo_salida, escenario_preset, fondo_descripcion, agregar_borde, activar_control_calidad,
            prompt_libre_personaje, semilla, num_imagenes, progress=progress,
        )
...
with gr.Blocks(title="Generador de Personajes Manga/Anime") as interfaz:
    ...
    with gr.Tab("1️⃣ Entrenar personaje"):
        nombre_e = gr.Textbox(label="Personaje", placeholder="rstchica")
        archivos_e = gr.File(label="➕", file_count="multiple", type="filepath",
                              file_types=["image"])
        log_agregar = gr.Textbox(label="Estado", interactive=False, lines=6)
        archivos_e.upload(_wrapper_agregar_y_entrenar, [archivos_e, nombre_e], log_agregar)

    with gr.Tab("2️⃣ Generar imágenes"):
        entrada_g = gr.Textbox(
            label="✍️ Escribí el personaje y qué querés generar",
            placeholder="rstchica: sentada en la playa comiendo un helado, sonriendo, atardecer de fondo, "
                        "con un perro de collar negro al lado",
            info="Formato: nombre_del_personaje: descripción libre (el nombre tiene que ser el mismo que usaste al entrenar)",
            lines=4,
        )

        with gr.Accordion("⚙️ Opciones de salida", open=False):
            with gr.Row():
            ...
            with gr.Row():
                ...
                fondo_desc_g = gr.Textbox(
                    label="Descripción del fondo (si elegiste 'Personalizado' arriba, o simplemente describilo en la casilla de arriba)",
                    placeholder="ej: salón de clases, atardecer en la ciudad, bosque",
                    visible=False,
                )
            control_calidad_g = gr.Checkbox(
                value=True,
                label="Control de calidad: detectar anomalías y reintentar automáticamente (heurístico, más lento)")
            with gr.Row():
                semilla_g = gr.Number(label="Semilla (vacío = aleatoria; usá la misma semilla para variar un detalle)", value=None)
        ...
        log_calidad_g = gr.Textbox(label="Control de calidad", interactive=False, lines=4)
        estado_ultima_generacion = gr.State({})

        with gr.Accordion("🛠️ Corregir y volver a generar", open=True):
            gr.Markdown(
                "Después de generar, escribí exactamente qué salió mal. Ejemplos: "
                "**una mano tiene seis dedos**, **aparecen tres brazos**, **la cara está deformada**, "
                "**falta una pierna**. La corrección conserva el prompt original y agrega la instrucción."
            )
            anomalia_g = gr.Textbox(
                label="¿Qué salió mal?",
                placeholder="Ej: la mano derecha tiene seis dedos y el brazo izquierdo está duplicado",
                lines=3,
            )
            with gr.Row():
                boton_corregir_g = gr.Button("🔧 Corregir y regenerar", variant="primary")
                boton_correcta_g = gr.Button("✅ Está correcta — guardar como ejemplo")
            log_correccion_g = gr.Textbox(label="Memoria de corrección", interactive=False, lines=4)
            with gr.Row():
                nombre_entrena_g = gr.Textbox(label="Personaje para actualizar LoRA", placeholder="rstchica")
                pasos_entrena_g = gr.Number(label="Pasos", value=1200, precision=0)
                rank_entrena_g = gr.Number(label="Rank", value=32, precision=0)
            boton_entrenar_actualizar_g = gr.Button("🧠 Entrenar / actualizar LoRA")
            log_entrenamiento_g = gr.Textbox(label="Estado del entrenamiento", interactive=False, lines=3)

        boton_generar.click(
            _wrapper_generar_simple,
            [entrada_g, tipo_salida_g, escenario_preset_g, fondo_desc_g, borde_sticker_g,
             control_calidad_g, semilla_g, num_imagenes_g],
            [galeria, semilla_usada_g, log_calidad_g, estado_ultima_generacion],
        )

        boton_corregir_g.click(
            corregir_ultima_generacion,
            [estado_ultima_generacion, anomalia_g, tipo_salida_g, borde_sticker_g, control_calidad_g],
            [galeria, semilla_usada_g, log_calidad_g, estado_ultima_generacion],
        ).then(
            lambda estado: "✅ Corrección registrada para '" + estado.get("nombre", "personaje") + "'." if estado else "",
            [estado_ultima_generacion],
            [log_correccion_g],
        )

        boton_correcta_g.click(
            aceptar_ultima_generacion_como_ejemplo, [estado_ultima_generacion],
            [log_correccion_g, estado_ultima_generacion]
        )
        boton_entrenar_actualizar_g.click(
            entrenar_actualizar_desde_dataset,
            [nombre_entrena_g, pasos_entrena_g, rank_entrena_g],
            [log_entrenamiento_g],
        )
    ...
    with gr.Tab("3️⃣ Generación libre"):
        ...
        log_libre_g = gr.Textbox(label="Estado", interactive=False)
        ...
        boton_generar_libre.click(
            ...
            [galeria_libre, semilla_usada_libre_g, log_libre_g],
        )
